...
import { PPEEngine, Rule, hashParams } from '@run-iq/core';
...
function createRule(id: string, model: string, params: any, extra: any = {}): Rule {
...
describe('THE FINAL BOSS: Gigafactory Togo Simulation', () => {
  ...
  const rules = [
    ...
    createRule('meta-startup', 'META_SHORT_CIRCUIT', { value: 0, reason: 'Startup Exemption' }, {
      priority: 9999,
      category: 'META',
    ...
    createRule('meta-zf', 'META_INHIBITION', { targetCategories: ['TVA', 'IS_STANDARD'] }, {
      priority: 9000,
      category: 'META',
    ...
    createRule('tva-std', 'FLAT_RATE', { base: 'revenue', rate: 0.18 }, {
      priority: 3000,
      category: 'TVA'
    }),

    // 4. IS Standard (Will be inhibited)
    createRule('is-std', 'FLAT_RATE', { base: 'profit', rate: 0.27 }, {
      priority: 3000,
    ...
    createRule('is-special-zf', 'FLAT_RATE', { base: 'profit', rate: 0.05 }, {
      priority: 3000,
      category: 'IS_SPECIAL'
    }),

    // 6. IRPP (Progressive)
    createRule('irpp', 'PROGRESSIVE_BRACKET', {
      base: 'taxable_salary',
      brackets: [
    ...
    // 8. Low Rate (Loser)
    createRule('tax-pro-low', 'FLAT_RATE', { base: 'revenue', rate: 0.01 }, {
      priority: 2000,
      category: 'TAXE_PRO'
    }),
    ...
    createRule('tax-pro-high', 'FLAT_RATE', { base: 'revenue', rate: 0.015 }, {
      priority: 2000,
      category: 'TAXE_PRO'
    }),

    // 10. Timbre (Fixed)
    createRule('timbre', 'FIXED_AMOUNT', { amount: 5000, currency: 'XOF' }, {
      priority: 1000,
      category: 'TIMBRE'
    }),

    // 11. Ghost Rule (Wrong Country)
    createRule('ghost-rule', 'FIXED_AMOUNT', { amount: 1000000, currency: 'XOF' }, {
      priority: 5000,
      country: 'BJ', // Benin
      category: 'TIMBRE'
    })
  ...
  it('runs the Full Simulation correctly', async () => {
    ...
    console.log('Final Trace:', JSON.stringify(result.trace.steps.map(s => `${s.ruleId}: ${s.reason || s.contribution}`), null, 2));
    console.log('Skipped:', JSON.stringify(result.skippedRules.map(s => `${s.rule.id} -> ${s.reason}`), null, 2));
    ...
    const taxProSkipped = result.skippedRules.find(s => s.rule.category === 'TAXE_PRO' && s.reason === 'RULE_CONFLICT');
